...
# a project keeps whatever its own tasks do around those calls (waiting for
# the db, pulling images). Plain compose is the fallback for a project
# without tasks.py, or a box without invoke (a server usually has neither).
_INVOKE_ACTIONS = ("start", "stop", "restart")
...
def _container_ip(container: str | None, host: Host = LOCAL) -> str | None:
    ...
    return next((ip for ip in out.split() if ip), None)
...
def pg_target_of(inst: Instance, host: Host = LOCAL) -> PgTarget:
    ...
    return PgTarget(
        host=_container_ip(inst.get("db_container"), host),
...
def render_config(config: Path, version: str | None, mode: str, host: Host = LOCAL) -> str:
    ...
    if host.container is not None:
        ...
        if not tmp:
            return "(could not create a temporary file to copy the config to)"

        if box.run(["docker", "cp", f"{host.container}:{config}", tmp]).returncode != 0:
