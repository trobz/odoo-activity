...
docker compose (all are merged), and needs the `odoo-db` CLI on `PATH` for the
...
- **`docker`** — a docker compose project running Odoo, doodba-shaped or
  ...
  `invoke start|stop|restart` when it has a `tasks.py` — doodba's, which is
  what a developer already drives it with — and fall back to
