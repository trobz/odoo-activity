...
def _no_neutralization_probe(monkeypatch):
    ...
    would still run `systemctl` and `odoo-db list` against whatever box the
    suite happens to run on -- slow, answering about the developer's own
    ...
    monkeypatch.setattr(tui, "neutralized_databases", lambda *_a, **_k: {})
